...
import numpy as np
import os
if not os.path.exists('./similarity_match/NYT_star'):
    os.makedirs('./similarity_match/NYT_star')
from similarities import Similarity
m = Similarity(model_name_or_path="sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2")
entity1_path='./data_decomposition/NYT_star/entity1.json'
entity2_path='./data_decomposition/NYT_star/entity2.json'
relation_Message_path='./data_decomposition/NYT_star/relation.json'

entity1_properity_path='./data_decomposition/NYT_star/entity1_properity.json'
entity2_properity_path='./data_decomposition/NYT_star/entity2_properity.json'
relation_Message_properity_path='./data_decomposition/NYT_star/relation._properityjson'
...
data_entity1_properity=json.load(open(entity1_properity_path))
data_entity2_properity=json.load(open(entity2_properity_path))
data_relation_Message_properity=json.load(open(relation_Message_properity_path))
        ...
        for u_number in range(len(data_entity1[ent1_index_one])):
            for v_number in range(len(data_entity1[ent1_index_two])):
                if data_entity1[ent1_index_one][u_number] in dicu_exist.keys() and data_entity1[ent1_index_two][v_number] in dicv_exist.keys():
                    continue
                if data_entity1_properity[ent1_index_one][u_number]!=data_entity2_properity[ent1_index_two][v_number]:
                    continue
        ...
        for u_number in range(len(data_entity2[ent2_index_one])):
            for v_number in range(len(data_entity2[ent2_index_two])):
                if data_entity2[ent2_index_one][u_number] in dicu_exist.keys() and data_entity2[ent2_index_two][v_number] in dicv_exist.keys():
                    continue
                if data_entity2_properity[ent2_index_one][u_number]!=data_entity2_properity[ent2_index_two][v_number]:
                    continue
        ...
        for u_number in range(len(data_relation_Message[relation_index_one])):
            for v_number in range(len(data_relation_Message[relation_index_two])):
                if data_relation_Message[relation_index_one][u_number] in dicu_exist.keys() and data_relation_Message[relation_index_two][v_number] in dicv_exist.keys():
                    continue
                if data_relation_Message_properity[relation_index_one][u_number]!=data_relation_Message_properity[relation_index_two][v_number]:
                    continue
...
with open('./similarity_match/NYT_star/sim_Entity1.json','w') as file_obj:
    json.dump(sim_Entity1,file_obj,ensure_ascii=False)
with open('./similarity_match/NYT_star/sim_Entity2.json','w') as file_obj:
    json.dump(sim_Entity2,file_obj,ensure_ascii=False)
with open('./similarity_match/NYT_star/sim_Relation_Message.json','w') as file_obj:
